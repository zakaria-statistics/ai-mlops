...
- **Agentfield** - Open-source Kubernetes and IAM for autonomous AI agents
...
- Agentfield control plane for scalable, secure agent microservices
- Workflow automation
...
### 9. Install Agentfield (Agent Control Plane)

```bash
# Agentfield provides Kubernetes-native orchestration for AI agents
# Install the Agentfield CLI
curl -sSL https://agentfield.ai/install.sh | sh

# Initialize Agentfield on the workbench
agentfield init

# Deploy a sample autonomous agent
agentfield deploy --image agentfield/sample-agent:latest
```
